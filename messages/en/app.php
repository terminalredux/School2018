<?php

return [
    'academic_title.academic_title' => 'Academic Title',
    'academic_title.academic_titles' => 'Academic Titles',
    'academic_title.create_academic_title' => 'Create Academic Title',
    'academic_title.created_at' => 'Created At',
    'academic_title.delete' => 'Delete',
    'academic_title.full' => 'Fullname',
    'academic_title.id' => 'ID',
    'academic_title.order' => 'Order',
    'academic_title.order_title' => 'Change order',
    'academic_title.reset' => 'Reset',
    'academic_title.save' => 'Save',
    'academic_title.search' => 'Search',
    'academic_title.short' => 'Shortcut',
    'academic_title.status' => 'Status',
    'academic_title.update' => 'Update',
    'academic_title.update_academic_title' => 'Update Academic Title',
    'academic_title.updated_at' => 'Updated at',
    'building.building' => 'Building',
    'building.buildings' => 'Buildings',
    'building.address' => 'Address',
    'building.gv_button_relations' => 'Relation with room types',
    'building.building_create' => 'Create Building',
    'building.building_update' => 'Update Building',
    'building.id' => 'ID',
    'building.name' => 'Name',
    'building.street' => 'Street',
    'building.street_number' => 'Street Number',
    'building.city' => 'City',
    'building.postcode' => 'Postcode',
    'building.description' => 'Description',
    'building.status' => 'Status',
    'building.created_at' => 'Created At',
    'building.updated_at' => 'Updated at',
    'building.select_building' => 'Select building',
    'consultation.consultation' => 'Consultation',
    'consultation.consultations' => 'Consultations',
    'consultation.add_consultation' => 'Add consultation',
    'consultation.id' => 'ID',
    'consultation.professor_id' => 'Professor ID',
    'consultation.room_id' => 'Room ID',
    'consultation.begin' => 'Begin',
    'consultation.end' => 'End',
    'consultation.additional_info' => 'Additional Info',
    'consultation.status' => 'Status',
    'consultation.public' => 'Public',
    'consultation.created_at' => 'Created At',
    'consultation.updated_at' => 'Updated At',
    'consultation.status_changed_term' => 'Changed term',
    'consultation.status_active' => 'Active',
    'consultation.status_cancel' => 'Cancel',
    'course.id' => 'ID',
    'course.course_basic_id' => 'Course Basic ID',
    'course.course_type_id' => 'Course Type ID',
    'course.major_id' => 'Major ID',
    'course.ects' => 'Ects',
    'course.total_hours' => 'Total Hours',
    'course.status' => 'Status',
    'course.create_course_basic' => 'Created At',
    'course.update_at' => 'Updated At',
    'course.course' => 'Course',
    'course.courses' => 'Courses',
    'course.create_course' => 'Create course',
    'course.update' => 'Update course',
    'course.add_course' => 'Add course',
    'course_basic.id'  => 'ID',
    'course_basic.name' => 'Name',
    'course_basic.status' => 'Status',
    'course_basic.create_at' => 'Create at',
    'course_basic.update_at' => 'Update at',
    'course_basic.create_course_basic' => 'Create basic course',
    'course_basic.update_course_basic' => 'Update basic course',
    'course_basic.course_basic' => 'Basic course',
    'course_basic.courses_basic' => 'Basic courses',
    'course_type.id' => 'ID',
    'course_type.type' => 'Type',
    'course_type.status' => 'Status',
    'course_type.create_at' => 'Create at',
    'course_type.update_at' => 'Update at',
    'course_type.course_type' => 'Course type',
    'course_type.course_types' => 'Course types',
    'course_type.create_course_type' => 'Create course type',
    'course_type.update_course_type' => 'Update course type',
    'department.id' => 'ID',
    'department.name' => 'Name',
    'department.description' => 'Description',
    'department.status' => 'Status',
    'department.created_at' => 'Created At',
    'department.updated_at' => 'Updated At',
    'department.department' => 'Department',
    'department.departments' => 'Departments',
    'department.create_department' => 'Create Department',
    'department.update_department' => 'Update Department',
    'department.short_name' => 'Short name',
    'professor.id' => 'id',
    'professor.academic_title_id' => 'Academic title',
    'professor.firstname' => 'First name',
    'professor.middlename' => 'Middle name',
    'professor.lastname' => 'Last name',
    'professor.gender' => 'Gender',
    'professor.website' => 'Website',
    'professor.email' => 'Email',
    'professor.status' => 'Status',
    'professor.created_at' => 'Created At',
    'professor.updated_at' => 'Updated at',
    'professor.create_professor' => 'Create Professor',
    'professor.update_professor' => 'Update Professor',
    'professor.professors' => 'Professors',
    'professor.create_professor' => 'Create Professor',
    'professor.male' => 'Male',
    'professor.female' => 'Female',
    'professor.fullname' => 'Full name',
    'room.id' => 'ID',
    'room.room_type_building_id' => 'Room Type Building ID',
    'room.number' => 'Number',
    'room.additional_info' => 'Additional Info',
    'room.created_at' => 'Created At',
    'room.updated_at' => 'Updated at',
    'room.status' => 'Status',
    'room.room' => 'Room',
    'room.rooms' => 'Rooms',
    'room.rooms_in' => 'Rooms in',
    'room.no_rooms' => 'No rooms', 
    'room.create_room' => 'Create room',
    'room.update_room' => 'Update room',
    'room_type.all_room_types' => 'All room types',
    'room_type.accepted_room_types' => 'Assigned to the building',
    'room_type.add_room_type' => 'Add new room types',
    'room_type.id' => 'ID',
    'room_type.type' => 'Type',
    'room_type.description' => 'Description',
    'room_type.status' => 'Status',
    'room_type.created_at' => 'Create Professor',
    'room_type.updated_at' => 'Update Professor',
    'room_type.room_type' => 'Room type',
    'room_type.room_types' => 'Room types',
    'room_type.create' => 'Create Room Type',
    'room_type.update' => 'Update Room Type',
    'room_type_building.id' => 'ID',
    'room_type_building.building_id' => 'Building ID',
    'room_type_building.room_type_id' => 'Room Type ID',
    'room_type_building.status' => 'Status',
    'room_type_building.create_room_type' => 'Create Room Type',
    'room_type_building.created_at' => 'Created At',
    'room_type_building.updated_at' => 'Updated At',
    'room_type_building.add_to_building' => 'Add to building',
    'navbar.academic_title' => 'Academic Titles',
    'navbar.building' => 'Buildings',
    'navbar.infrastructure' => 'Infrastructure',
    'navbar.professor' => 'Professors',
    'navbar.room_type' => 'Room Types',
    'major.major' => 'Major',
    'major.majors' => 'Majors',
    'major.id' => 'ID',
    'major.department_id' => 'Department ID',
    'major.name' => 'Name',
    'major.description' => 'Description',
    'major.status' => 'Status',
    'major.created_at' => 'Created at',
    'major.updated_at' => 'Updated at',
    'major.create_major' => 'Create major',
    'major.update_major' => 'Update major',
    'major.choose_department' => 'Department',
    'system.school2018' => 'School 2018',
    'system.add' => 'Add',
    'system.create' => 'Create',
    'system.not_set' => 'Not set',
    'system.save' => 'Save',
    'system.save_changes' => 'Save changes',
    'system.update' => 'Update',
    'system.search' => 'Search',
    'system.reset' => 'Reset',
    'system.delete' => 'Delete',
    'system.confirm' => 'Are you sure you want to delete this item?',
    'system.public' => 'Public',
    'system.publish' => 'Publish',
    'system.not_public' => 'Not public',
    'system.status' => 'Status',
    'system.unpublish' => 'Unpublish',
    'system.week_odd' => 'Odd',
    'system.week_even' => 'Even',
    
];